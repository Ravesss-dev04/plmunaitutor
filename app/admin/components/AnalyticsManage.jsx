...
const AnalyticsManage = () => {
  ...
  useEffect(() => {
    const fetchTeacherActivity = async () => {
      try {
        const response = await fetch('/api/admin/teacher-activity');
        if (response.ok) {
          const data = await response.json();
          setTeacherActivityData(data);
        }
      } catch (error) {
    ...
    const fetchWeeklyUsage = async () => {
      try {
        const response = await fetch('/api/admin/weekly-usage');
        if (response.ok) {
          const data = await response.json();
  ...
  }, [loading, teacherActivityData, weeklyUsageData]);
