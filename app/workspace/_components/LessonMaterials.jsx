...
import { File, FileText, Video, BookOpen, ExternalLink, CheckCircle2, Clock } from "lucide-react";
...
function LessonMaterials() {
  ...
  const getIcon = (type) => {
    ...
    return (
      <div className="relative">
        <div className="absolute inset-0 bg-blue-500/20 rounded-lg blur-sm"></div>
        <File className="relative text-blue-400 w-5 h-5 sm:w-6 sm:h-6 shrink-0" />
      </div>
    );
  };

  // Flatten lessons with course info
  const allLessons = courseData.flatMap((course) =>
    course.lessons?.map((lesson, index) => ({
      ...lesson,
      courseTitle: course.title,
      courseId: course.id,
      globalIndex: `${course.id}-${index}`,
    })) || []
  );
